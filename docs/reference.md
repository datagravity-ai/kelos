...
| `spec.when.slack.triggers[].pattern` | RE2 regex matched against message text (unanchored); bot mention required unless `mentionOptional` is set; multiple triggers use OR semantics; when empty, every bot mention fires | No |
| `spec.when.slack.triggers[].mentionOptional` | When `true`, fire on pattern match alone without requiring a bot @-mention | No |
| `spec.when.slack.excludePatterns` | RE2 regex patterns that reject messages when any pattern matches (OR semantics); leading @-mentions are stripped before matching; does not apply to slash commands | No |
