...
A `WebhookGateway` is an authentication and routing boundary for webhook-driven
TaskSpawners and SessionSpawners. Each gateway:
...
- fans out only to spawners **in its own namespace** that reference it via
...
- `generic` gateways are accepted but **not signature-verified** (the gateway's
  `status.phase` is `Unauthenticated`); restrict access at the network layer.
- Spawners with a `gatewayRef` are served exclusively by the gateway; the
