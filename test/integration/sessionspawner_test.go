...
var _ = Describe("SessionSpawner", func() {
	...
	It("rejects a Slack source with no idle policy", func() {
		spawner := validSlackSessionSpawner(namespace, "slack-no-idle")
		spawner.Spec.SessionTemplate.IdlePolicy = nil
		err := k8sClient.Create(ctx, spawner)
		Expect(apierrors.IsInvalid(err)).To(BeTrue(), "error: %v", err)
		Expect(err.Error()).To(ContainSubstring("idlePolicy must set suspendAfterSeconds or deleteAfterSeconds"))
	})

	It("rejects a Slack source whose idle policy sets neither bound", func() {
		spawner := validSlackSessionSpawner(namespace, "slack-empty-idle")
		spawner.Spec.SessionTemplate.IdlePolicy = &kelos.SessionIdlePolicy{}
		err := k8sClient.Create(ctx, spawner)
		Expect(apierrors.IsInvalid(err)).To(BeTrue(), "error: %v", err)
	})

	It("accepts a Slack source that only deletes", func() {
		spawner := validSlackSessionSpawner(namespace, "slack-delete-only")
		deleteAfter := int32(86400)
		spawner.Spec.SessionTemplate.IdlePolicy = &kelos.SessionIdlePolicy{DeleteAfterSeconds: &deleteAfter}
		Expect(k8sClient.Create(ctx, spawner)).To(Succeed())
	})

	It("accepts a Slack source that suspends then deletes", func() {
		spawner := validSlackSessionSpawner(namespace, "slack-suspend-delete")
		suspendAfter := int32(1800)
		deleteAfter := int32(86400)
		spawner.Spec.SessionTemplate.IdlePolicy = &kelos.SessionIdlePolicy{
	...
	It("does not require an idle policy for a GitHub webhook source", func() {
		spawner := validSessionSpawner(namespace, "webhook-no-idle")
		spawner.Spec.SessionTemplate.IdlePolicy = nil
		Expect(k8sClient.Create(ctx, spawner)).To(Succeed())
	})
...
func validSlackSessionSpawner(namespace, name string) *kelos.SessionSpawner {
	...
	spawner.Spec.SessionTemplate.InitialPrompt = ""
	suspendAfter := int32(1800)
	spawner.Spec.SessionTemplate.IdlePolicy = &kelos.SessionIdlePolicy{SuspendAfterSeconds: &suspendAfter}
	return spawner
}
