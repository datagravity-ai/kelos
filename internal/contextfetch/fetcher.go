...
func (f *Fetcher) FetchAll(ctx context.Context, sources []v1alpha1.ContextSource, templateVars map[string]interface{}) (map[string]interface{}, error) {
	...
	for _, src := range sources {
		g.Go(func() error {
			...
			if err != nil {
				if src.Required {
					return fmt.Errorf("required context source %q: %w", src.Name, err)
				}
				if gctx.Err() != nil {
					// Context was cancelled (likely by a required source failing);
...
func (f *Fetcher) fetchOne(ctx context.Context, src v1alpha1.ContextSource, templateVars map[string]interface{}) (string, error) {
	// Render URL template
	renderedURL, err := renderTemplateStr("url", src.URL, templateVars)
	if err != nil {
		return "", fmt.Errorf("rendering URL template: %w", err)
	}

	// Validate URL scheme
	if err := validateURLScheme(renderedURL, src.AllowInsecure); err != nil {
		return "", err
	}

	// Resolve headers
	headers, err := f.resolveHeaders(ctx, src, templateVars)
	if err != nil {
	...
	if src.Body != "" {
		rendered, err := renderTemplateStr("body", src.Body, templateVars)
		if err != nil {
	...
	if src.TimeoutSeconds != nil {
		timeoutSec = int(*src.TimeoutSeconds)
	}

	maxBytes := int64(defaultMaxResponseBytes)
	if src.MaxResponseBytes != nil {
		maxBytes = int64(*src.MaxResponseBytes)
	}
	...
	if src.JSONPathFilter != "" {
		return applyJSONPathFilter(body, src.JSONPathFilter)
	}

	return string(body), nil
}

func (f *Fetcher) resolveHeaders(ctx context.Context, src v1alpha1.ContextSource, templateVars map[string]interface{}) (map[string]string, error) {
	headers := make(map[string]string, len(src.Headers)+len(src.HeadersFrom))
	...
	for _, hfs := range src.HeadersFrom {
		secret := &corev1.Secret{}
		key := client.ObjectKey{Name: hfs.SecretRef.Name, Namespace: f.Namespace}
		if err := f.Client.Get(ctx, key, secret); err != nil {
			return nil, fmt.Errorf("reading Secret %q for header %q: %w", hfs.SecretRef.Name, hfs.Header, err)
		}
		val, ok := secret.Data[hfs.Key]
		if !ok {
			return nil, fmt.Errorf("key %q not found in Secret %q", hfs.Key, hfs.SecretRef.Name)
		}
