...
func TestSessionBridgeDoesNotWarnOnACompletedTurn(t *testing.T) {
	// Codex records a turn-scoped error for a request it then retries, so a
	// turn can complete successfully with result.Message set. A correct answer
	// must not carry a warning under it.
	scheme := sessionScheme(t)
	spawner := testSessionSpawner()
	cl := fake.NewClientBuilder().WithScheme(scheme).WithObjects(spawner).Build()
	...
	driver := &fakeDriver{results: []sessionturn.Result{{
		TurnID:  "turn-1",
		Status:  sessionturn.StatusCompleted,
	...
	posts := poster.recorded()
	if len(posts) != 2 {
		t.Fatalf("recorded %d Slack writes, want two: %+v", len(posts), posts)
	}
	if !strings.Contains(posts[1].text, "the build is green") {
		t.Errorf("reply = %q, want the assistant answer", posts[1].text)
	}
	if strings.Contains(posts[1].text, "retrying") {
		t.Errorf("reply = %q, want no error under a completed turn", posts[1].text)
	}
}

func TestTurnPromptDoesNotResendTheThreadForABareMention(t *testing.T) {
	// Body holds the whole formatted thread on a reply, including messages
	// that never matched, so it must never be the fallback for a later turn.
	msg := &SlackMessageData{
		UserID: "U1", UserName: "Hans", ChannelID: "C1",
		Text:     "<@UBOT>",
		Body:     "the whole formatted thread, including unmatched replies",
		ThreadTS: "1790000000.000100",
	}
	prompt := turnPrompt(msg, false)
	if strings.Contains(prompt, "the whole formatted thread") {
		t.Errorf("prompt = %q, want the thread not resent for a bare mention", prompt)
	}
	if strings.TrimSpace(prompt) == "" {
		t.Error("prompt is empty, which the runtime rejects")
	}
}
