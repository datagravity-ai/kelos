...
func (s *SlackSource) handleEventsAPI(sm *socketmode.Client, evt socketmode.Event) {
	...
	body, ok := shouldProcess(innerEvent.User, innerEvent.SubType, innerEvent.ThreadTimeStamp, innerEvent.Text, s.selfUserID)
	if !ok {
		return
	}
...
func shouldProcess(userID, subtype, threadTS, text, selfUserID string) (string, bool) {
