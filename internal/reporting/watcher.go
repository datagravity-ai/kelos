...
const (
	...
	// LabelSlackReporting is the label used to efficiently list Tasks that
	// have Slack reporting enabled, avoiding full cluster scans.
...
// SweepCaches removes entries for tasks that are no longer active.
...
func (tr *SlackTaskReporter) SweepCaches(activeUIDs map[types.UID]bool) {
	tr.mu.Lock()
	defer tr.mu.Unlock()
